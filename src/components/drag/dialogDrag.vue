<template>
  <div v-if="visible" class="my_dialog">
    ...
    <div class="my_dialog_box" :style="{width:width+'px'}" id="my_dialog_box" v-drag>
      <!-- 标题 -->
      ...
      <div class="my_dialog_bottom">
        ...
        <button class="btn rebutBtn" v-if="showRebutBtn" @click="rebut">{{rebuttext}}</button>
...
export default {
  props: {
    ...
    rebuttext: { type: String, default: '驳回' }, //确认按钮文案，默认 驳回
    ...
    showRebutBtn: { type: Boolean, default: false }, //是否显示驳回按钮，false 否，true 是，默认 false
...
.my_dialog_bottom .rebutBtn:focus,
.my_dialog_bottom .rebutBtn:hover {
  background: #df6c76;
  border: 1px solid #66b1ff;
  color: #fff;
}
