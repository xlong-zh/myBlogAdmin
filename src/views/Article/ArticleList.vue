    ...
    <el-table :data="model" v-if="model">
      ...
      <el-table-column prop="icon" label="图标" width="100">
        <template slot-scope="scope">
          <img @click="bigPicture(scope.row.icon)" :src="scope.row.icon" style="height:50px;cursor: pointer;" alt />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <el-button v-has @click="$router.push(`/article/edit/${scope.row._id}`)" type="text" size="small">编辑</el-button>
