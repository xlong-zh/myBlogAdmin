    ...
    <el-table v-if="items" :data="items">
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <el-button v-has="'visitor'" @click="$router.push(`/category/edit/${scope.row._id}`)" type="text" size="small"
            >编辑</el-button
          >
  ...
  methods: {
    ...
    async remove(row) {
      this.$confirm(`是否确认删除${row.name}`, '提示', {
        ...
        .then(async () => {
          const res = await this.$http.deleteAction(`/rest/category/${row._id}`);
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
